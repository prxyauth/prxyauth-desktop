...
export function useProxyProviders() {
  ...
  const fetchProviders = useCallback(async () => {
    try {
      ...
      const message =
        err instanceof ApiError ? err.message : "Failed to fetch providers";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const save = useCallback(
    async (data: SaveProviderRequest): Promise<boolean> => {
      try {
        setIsSaving(true);
        setError(null);
  ...
  const remove = useCallback(
    async (provider: ProxyProviderType): Promise<boolean> => {
      try {
        ...
        const message =
          err instanceof ApiError ? err.message : "Failed to remove provider";
        setError(message);
        return false;
      } finally {
        setIsRemoving(null);
  ...
  const toggle = useCallback(
    async (provider: ProxyProviderType, enabled: boolean): Promise<boolean> => {
      try {
        setError(null);
        ...
        const message =
          err instanceof ApiError ? err.message : "Failed to toggle provider";
        setError(message);
        return false;
      }
    },
  ...
  const getProvider = useCallback(
    (type: ProxyProviderType): ProxyProviderConfig | undefined => {
