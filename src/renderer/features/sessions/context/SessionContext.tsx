...
export function SessionProvider({ children }: { children: ReactNode }) {
    ...
    const [transitioningSessions, setTransitioningSessions] = useState<Record<string, { status: string; logs: string[] }>>({});
    const [openBrowsers, setOpenBrowsers] = useState<string[]>([]);

    const setSessionLog = useCallback((sessionId: string, status: string | null, logEntry?: string) => {
    ...
    const logout = useCallback(async (sessionId: string): Promise<boolean> => {
        try {
            ...
            const message = err instanceof ApiError ? err.message : "Failed to logout";
            setError(message);
            return false;
        } finally {
    ...
    const verify = useCallback(async (sessionId: string): Promise<boolean> => {
        try {
            ...
            const message = err instanceof ApiError ? err.message : "Failed to verify session";
            setError(message);
            return false;
        } finally {
            setVerifyingId(null);
    ...
    const open = useCallback(async (sessionId: string): Promise<boolean> => {
        try {
            ...
            const message = err instanceof ApiError ? err.message : "Failed to open session";
            setError(message);
            return false;
        } finally {
            setIsOpening(null);
    ...
    const launchSessionBrowser = useCallback(async (sessionId: string) => {
        ...
        try {
            ...
            setSessionLog(sessionId, "Authentication Check", "Retrieving credentials from vault...");
            const session = await sessionApi.get(sessionId);
            if (!session) throw new Error("Session not found");

            setSessionLog(sessionId, "Data Sync", "Exporting browser storage state...");
            const storageState = await sessionApi.exportBrowserState(sessionId);

            setSessionLog(sessionId, "Launching Chromium", "Applying anti-detection fingerprints...");
            ...
            setSessionLog(sessionId, "Connection Ready", "Secure proxy tunnel established");
    ...
    const closeSessionBrowser = useCallback(async (sessionId: string) => {
        // Optimistic UI update
        setOpenBrowsers(prev => prev.filter(id => id !== sessionId));
        ...
        try {
            ...
            const result = await (window as any).prxApi.playwright.closeLocal(sessionId);
            if (!result.success) {
                throw new Error(result.error || "Failed to close browser");
            }
            setSessionLog(sessionId, "Cleanup In Progress", "Closing proxy bridge and releasing resources...");
        } catch (err) {
            // Revert optimistic update
            setOpenBrowsers(prev => [...new Set([...prev, sessionId])]);
            setSessionLog(sessionId, null);

            const message = err instanceof Error ? err.message : "Failed to close browser";
            setError(message);
        } finally {
            setIsClosing(null);
        }
    ...
    const showSessionPortal = useCallback(async (sessionId: string) => {
        try {
            ...
            const message = err instanceof Error ? err.message : "Failed to show portal";
            setError(message);
        }
    }, [sessions]);
    ...
    useEffect(() => {
        ...
        const listenerCleanup = (window as any).prxApi.playwright.onBrowserClosed((sessionId: string) => {
        ...
        const syncBrowsers = async () => {
            try {
                const result = await (window as any).prxApi.playwright.listConnections();
