...
import { googleApi, officeApi, githubApi, ApiError } from "@core/api/client";
...
export function ConnectAccountModal({
  ...
  const handleSubmitPassword = async (e: FormEvent) => {
    ...
    try {
      ...
      if (provider === "GOOGLE" && sessionId) {
        ...
        response = await githubApi.login({
          username: email,
          password,
          fingerprint,
        });
  ...
  const handle2FASubmit = async (e: FormEvent) => {
    ...
    try {
      let response: LoginResponse;
      if (provider === "GOOGLE") {
        response = await googleApi.submit2FA({
          sessionId,
          code: twoFactorCode,
        });
      } else if (provider === "OFFICE") {
        response = await officeApi.submit2FA({
          sessionId,
          code: twoFactorCode,
        });
      } else {
        response = await githubApi.submit2FA({
          sessionId,
          code: twoFactorCode,
        });
      }
  ...
  const handleSwitchMethod = async (method: string) => {
    ...
    try {
      ...
      if (provider === "GITHUB") {
        ...
        throw new Error(
          "Switching methods not supported for this provider yet",
        );
      }
      handleLoginResponse(response);
    } catch (err) {
  ...
  const handleLoginResponse = (response: LoginResponse) => {
    if (response.success) {
      ...
      if (response.status === "AUTHENTICATED") {
        ...
        response.status === ("challenge_required" as any) ||
        response.status === ("requires_2fa" as any)
      ) {
        setStep("challenge");
              ...
              <div className="space-y-8">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm">
                    Select a provider to create a new session
                  </p>
                </div>
            ...
            {step === "password" && provider && (
              <form onSubmit={handleSubmitPassword} className="space-y-8">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm">
                    Provide your account password to proceed
                  </p>
                </div>
              ...
              <div className="space-y-8">
                  ...
                  <div className="inline-flex p-3 rounded-2xl bg-blue-500/10 border border-blue-500/20 mb-4">
                    <challengeUI.icon
                      className={cn(
                        ...
                        challengeType === "PUSH" && "animate-bounce",
                      )}
                    />
                  </div>
                  <h2 className="text-2xl font-bold text-white mb-2">
                ...
                {challengeType === "PUSH" ? (
                  <div className="mt-8 flex flex-col items-center">
                    {challengeMetadata?.pushCode ? (
                      <>
                        <div className="text-[10px] text-gray-500 uppercase tracking-[0.2em] font-bold mb-4">
                          Verification Code
                        </div>
                        <div className="relative w-24 h-24 rounded-full border-4 border-blue-500/30 flex items-center justify-center bg-white/5 shadow-[0_0_30px_rgba(96,165,250,0.2)]">
                          <span className="text-5xl font-bold text-white tracking-widest leading-none">
                            {challengeMetadata.pushCode}
                          </span>
                        </div>
                        <p className="mt-6 text-xs text-gray-500 text-center">
                          Tap the number above on your phone to sign in.
                        </p>
                      </>
                    ) : (
                ...
                {challengeMetadata?.availableMethods?.length > 1 && (
                  <div className="pt-6 border-t border-white/5">
                    <p className="text-[10px] text-center text-gray-500 uppercase tracking-widest font-bold mb-3">
                      Try another method
                    </p>
